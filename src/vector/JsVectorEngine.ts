...
function reconstructFlatArtwork(input: ImageData, options: VectorizeOptions): ReconstructedArtwork {
  ...
  // Logos frequently contain very thin taglines, trademark marks and hairline rules.
  // Use a gentler foreground threshold than scans/signatures so these survive.
  const distanceThreshold = options.preset === 'logo' ? 24 : options.preset === 'signature' ? 27 : 28;
  const strongThreshold = distanceThreshold + (options.preset === 'logo' ? 12 : 17);
  const foregroundMask = new Uint8Array(width * height);
  const strongPixels: Rgb[] = [];

  for (let p = 0, i = 0; p < foregroundMask.length; p += 1, i += 4) {
    if (source[i + 3] < 12) continue;
    ...
    const foreground = distance >= distanceThreshold || (chromaValue >= 24 && luminanceValue < 250);
    if (!foreground) continue;
    foregroundMask[p] = 1;
    if (distance >= strongThreshold || chromaValue >= 38) strongPixels.push(pixel);
  }
...
function buildAdaptiveBrandPalette(pixels: Rgb[], maxColors: number): Rgb[] {
  ...
  while (selected.length < maxColors) {
    ...
    for (const candidate of candidates) {
      ...
      const score = candidate.count * (1 + chroma(candidate.color) / 180) * Math.pow(Math.min(1.8, minDistance / 100), 1.4);
      if (score > bestScore) { best = candidate; bestScore = score; }
    }
    if (!best) break; selected.push(best.color);
  }
  const buckets = selected.map(() => [] as Rgb[]);
  for (let index = 0; index < pixels.length; index += step) buckets[nearestIndex(pixels[index], selected)].push(pixels[index]);
...
function supersampleFlatArtwork(source: ImageData, palette: Rgb[], factor: number, options: VectorizeOptions): ImageData {
  ...
  // Keep logo resampling nearly unblurred; excessive blur was thickening type and
  // erasing the small tagline. Signatures can tolerate a little more smoothing.
  context.filter = options.preset === 'logo' ? 'blur(0.12px)' : options.preset === 'signature' ? 'blur(0.24px)' : 'blur(0.28px)';
  ...
  const alphaFloor = options.preset === 'logo' ? 58 : options.preset === 'signature' ? 72 : 78;
...
function cleanMask(mask: Uint8Array, width: number, height: number, preserveFineDetail = false): void {
  ...
  for (let y = 1; y < height - 1; y += 1) for (let x = 1; x < width - 1; x += 1) {
    ...
    if (first[p] && neighbours <= (preserveFineDetail ? 0 : 1)) mask[p] = 0;
    else if (!first[p] && neighbours >= (preserveFineDetail ? 8 : 7)) mask[p] = 1;
  }
  removeTinyComponents(mask, width, height, preserveFineDetail ? 2 : 5);
}
...
function removeTinyComponents(mask: Uint8Array, width: number, height: number, minimumSize: number): void {
  ...
  for (let start = 0; start < mask.length; start += 1) {
    ...
    while (head < tail) {
      const p = queue[head++]; component.push(p); const x = p % width; const y = Math.floor(p / width);
      for (let dy = -1; dy <= 1; dy += 1) for (let dx = -1; dx <= 1; dx += 1) {
        if (dx === 0 && dy === 0) continue; const nx = x + dx, ny = y + dy;
        if (nx < 0 || ny < 0 || nx >= width || ny >= height) continue; const np = ny * width + nx;
        if (!mask[np] || visited[np]) continue; visited[np] = 1; queue[tail++] = np;
      }
    }
...
function toTraceOptions(options: VectorizeOptions, traceScale = 1, traceColors = options.colors) {
  const detail = options.detail / 100; const smoothing = options.smoothing / 100; const cleanGeometry = isFlatArtwork(options); const logo = options.preset === 'logo';
  const traceTolerance = logo ? Math.max(0.52, 1.42 - detail * 0.72) : cleanGeometry ? Math.max(0.72, 2.05 - detail * 1.08) : Math.max(0.18, 2.2 - detail * 1.9);
  return {
    ltres: traceTolerance, qtres: traceTolerance,
    pathomit: logo ? Math.max(1, Math.round((1 - detail) * 4)) : cleanGeometry ? Math.max(3, Math.round((1 - detail) * 10)) : Math.max(0, Math.round((1 - detail) * 12)),
    rightangleenhance: logo || options.preset === 'line-art', colorsampling: 0,
    numberofcolors: cleanGeometry ? traceColors : options.colors,
    mincolorratio: logo ? 0.0015 : cleanGeometry ? 0.004 : 0,
    colorquantcycles: logo ? 3 : cleanGeometry ? 2 : options.colors <= 4 ? 2 : 3,
    layering: 0, strokewidth: 0, linefilter: logo ? false : cleanGeometry || smoothing > 0.65,
    scale: traceScale, roundcoords: logo ? 3 : cleanGeometry ? 2 : detail > 0.8 ? 2 : 1,
    viewbox: true, desc: false, blurradius: 0, blurdelta: logo ? 18 : cleanGeometry ? 34 : 20,
  };
}
