...
/** Clean geometry + high-contrast foreground pipeline for centred banded badges/seals. */
export function tryVectorizeCenteredBandedBadge(source: ImageData, options: VectorizeOptions): string | null {
  ...
  const traced=ImageTracer.imagedataToSVG(mask,{
    ltres:0.055,qtres:0.075,pathomit:0,rightangleenhance:true,colorsampling:0,
    numberofcolors:2,mincolorratio:0,colorquantcycles:1,layering:0,strokewidth:0,
    linefilter:false,scale:1,roundcoords:8,viewbox:true,desc:false,blurradius:0,blurdelta:0,
  });
  const details=darkMaskPaths(traced);
  ...
  // The cleaned mask represents the high-contrast insignia as one neutral foreground.
  // Sample its dominant source colour so white/cream/gold line art is restored naturally.
...
function foregroundMask(source:ImageData,cx:number,cy:number,r:number,bands:Band[]):ImageData{
  ...
  for(let y=0;y<h;y++)for(let x=0;x<w;x++){
    ...
    // Strong chromatic OR luminance separation from the locally reconstructed band.
    if(dl>78 && (lumDelta>42 || dl>118))raw[y*w+x]=1;
  }
  // Morphological majority clean: remove one-pixel raster noise while retaining thin line art.
  const clean=new Uint8Array(raw);
  for(let y=1;y<h-1;y++)for(let x=1;x<w-1;x++){
    let n=0; for(let yy=-1;yy<=1;yy++)for(let xx=-1;xx<=1;xx++)if(raw[(y+yy)*w+x+xx])n++;
    const i=y*w+x;
    if(raw[i]&&n<=2)clean[i]=0;
    else if(!raw[i]&&n>=7)clean[i]=1;
  }
  // A second support pass removes tiny detached anti-alias fragments, not legitimate strokes.
  const final=new Uint8Array(clean);
  for(let y=1;y<h-1;y++)for(let x=1;x<w-1;x++)if(clean[y*w+x]){
    let orth=0; if(clean[y*w+x-1])orth++;if(clean[y*w+x+1])orth++;if(clean[(y-1)*w+x])orth++;if(clean[(y+1)*w+x])orth++;
    if(orth===0)final[y*w+x]=0;
  }
  const out=new ImageData(w,h);
  for(let i=0;i<w*h;i++){const v=final[i]?0:255,j=i*4;out.data[j]=v;out.data[j+1]=v;out.data[j+2]=v;out.data[j+3]=255;}
  return out;
}

function dominantInsigniaColor(source:ImageData,cx:number,cy:number,r:number,bands:Band[]):Rgb{
  const colors:Rgb[]=[];
  for(let y=Math.max(0,Math.floor(cy-r));y<Math.min(source.height,Math.ceil(cy+r));y+=2)for(let x=Math.max(0,Math.floor(cx-r));x<Math.min(source.width,Math.ceil(cx+r));x+=2){
    if(Math.hypot(x-cx,y-cy)>r*.94)continue; const band=bands.find(v=>y>=v.y0&&y<v.y1);if(!band)continue;
    const c=px(source,x,y);if(dist(c,band.color)>100&&Math.abs(luma(c)-luma(band.color))>45)colors.push(c);
  }
  if(!colors.length)return {r:255,g:255,b:255};
  // Prefer the brightest substantial cluster: typical badge line-art is white/cream/gold.
  const sorted=colors.sort((a,b)=>luma(b)-luma(a));return median(sorted.slice(0,Math.max(8,Math.floor(sorted.length*.35))));
}
